    ...
    def run(self):
        ...
        while True:
            ...
            if ret:
                ...
                p = convertToQtFormat.scaled(1800, 1200, Qt.KeepAspectRatio)
                self.changeGrpah.emit(p)
...
class QImageViewer(QMainWindow):
    ...
    def select_folder(self):
        ...
        # fileName, _ = QFileDialog.getOpenFileName(self, 'QFileDialog.getOpenFileName()', '',
        #                                           'Images (*.png *.jpeg *.jpg *.bmp *.gif)', options=options)
        self.folder = QFileDialog.getExistingDirectory(self, 'Select folder of screenshots', options=options)
        print(self.folder)
...
if __name__ == '__main__':
    ...
    with open("Blacklist.tsv", "a+") as blacklist:       
        app = QApplication(sys.argv)
        imageViewer = QImageViewer(blacklist)
        imageViewer.show()
        sys.exit(app.exec_())
